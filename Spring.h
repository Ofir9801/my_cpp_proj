...
public:
    Spring(Point start, int len, Keys dir, Screen& screen)//constractor
        : startPos(start), length(len), pushDirection(dir), board(screen) {}
	...
	void draw(const Point& playerPos, bool active = false) const;//function to draw the functioning of the spring on the board
